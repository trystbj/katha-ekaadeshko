...
- Improve dialogue and atmosphere while staying in ${langDisp}.
...
export function buildScriptPrompt({ story, input, region }) {
  ...
  const langDisp =
    String(input?.__storyLanguageDisplay || '').trim() ||
    languageDisplayName(input?.storyLanguage)
  return `${preamble}Convert this story into a cinematic short-form script suitable for 40s–2min video.
